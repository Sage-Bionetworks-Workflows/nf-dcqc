...
- Do NOT change `nextflow_schema.json`'s `$schema` to JSON Schema draft 2020-12 or rename `definitions` to `$defs`, even though `nf-core lint`'s `schema_lint` check demands it as a CRITICAL error. `lib/NfcoreSchema.groovy` (the legacy validator this repo actually runs, not the `nf-schema` plugin) hardcodes reads of the `definitions` key and loads the schema via the `org.everit.json.schema` library, which does not support draft 2020-12. Making that change breaks every pipeline run at startup with `ERROR ~ #: could not determine version` (confirmed in CI run 35399986512, PR #24) — since `params.validate_params = true` by default, this isn't optional code path. Keep the schema on draft-07/`definitions`; `nf-core lint`'s schema_lint failure here is expected and part of the same blocked `nf-schema` migration as the bullet above, not an independently fixable bug. As of the `.nf-core.yml` `lint.schema_lint: False` and `lint.schema_params: False` entries, both checks are skipped in CI rather than shown as a failure — do not re-enable either without also solving the draft-2020-12 problem above. `schema_params` must stay skipped alongside `schema_lint`: it calls the same `PipelineSchema.load_lint_schema()` draft check but, unlike `schema_lint`, does not catch the resulting `AssertionError`, so leaving it enabled aborts the entire `nf-core lint` run as an uncaught critical error rather than just failing one check.
...
- Jira DPE-1808, GitHub PR #22 ("Updated workflow to work with newer versions of Nextflow"), PR #25 ("Cap Nextflow version at 25.10.0"), PR #24 ("Fix linting issues" — attempted and reverted the `nextflow_schema.json` draft 2020-12 change for the reason above) — active, unresolved threads on the version-compatibility problem. Check these before touching `nextflow.config`, `main.nf`, `lib/`, or `nextflow_schema.json`.
